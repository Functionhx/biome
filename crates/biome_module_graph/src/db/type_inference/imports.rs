...
enum ExportResolution<'db> {
    ...
    Missing,
    /// One export was found.
    Resolved(ResolvedExport<'db>),
    /// More than one star export provides different matches.
    Ambiguous,
    /// The step limit prevented a reliable answer.
    Indeterminate,
}
...
impl<'db> ResolutionCtx<'db, '_> {
    ...
    fn resolve_export_name_result(
        ...
        seen.insert((InferredModuleKey::new(module.as_id()), name.to_string()));
        ...
        while let Some((module, name)) = stack.pop() {
            if !seen.insert((InferredModuleKey::new(module.as_id()), name.clone())) {
                continue;
            }
