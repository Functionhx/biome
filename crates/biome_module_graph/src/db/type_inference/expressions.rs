...
use biome_js_type_info::{
    ...
    resolved::{
        ...
        InferredConstructor, InferredFunction, InferredFunctionParameter,
        InferredGenericTypeParameter, InferredLiteral, InferredLiteralValue,
        InferredLocalTypeHandle, InferredNamedFunctionParameter, InferredReturnType, InferredTuple,
        InferredTupleElementType, InferredTypeData, InferredTypeMember, InferredTypeSubstitution,
...
impl std::str::FromStr for PromiseStaticMethod {
    type Err = ();

    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value {
            "all" => Ok(Self::All),
            "allSettled" => Ok(Self::AllSettled),
            "any" => Ok(Self::Any),
            "race" => Ok(Self::Race),
...
impl<'db> ResolutionCtx<'db, '_> {
    ...
    pub(in crate::db::type_inference) fn resolve_static_member_expression(
        ...
        match object {
            InferredTypeData::Class(_) => {
                if object.is_promise_class(self.db)
                    && matches!(member_name.parse().ok(), Some(PromiseStaticMethod::Resolve))
                {
                    return Some(self.promise_resolve_type(object));
                }
            ...
            InferredTypeData::InstanceOf(instance) => {
                ...
                // This branch resolves properties on a Promise instance. Static
                // methods are resolved from the Promise class instead.
    ...
    /// Builds the type of a method read from a Promise instance.
    ///
    /// The receiver's value type is preserved by `finally`, transformed by
    /// `then`, and combined with a recovery value by `catch`.
    ///
    /// These synthetic generics mirror TypeScript's Promise declarations:
    /// <https://github.com/microsoft/TypeScript/blob/637d5746b70257028fb95aad32ddec6b26ab0a14/src/lib/es5.d.ts#L1539-L1546>.
    fn promise_instance_method_type(
        ...
        match method {
            ...
            PromiseInstanceMethod::Then => {
                let fulfillment_handler_value = generic("TFulfillmentHandlerValue", receiver_value);
                let rejection_handler_value =
                    generic("TRejectionHandlerValue", InferredTypeData::NeverKeyword);
                let on_fulfilled_return = InferredTypeData::union_from_types(
                    self.db,
                    Vec::from([
                        fulfillment_handler_value,
                        promise(fulfillment_handler_value),
                    ]),
                );
                let on_rejected_return = InferredTypeData::union_from_types(
                    self.db,
                    Vec::from([rejection_handler_value, promise(rejection_handler_value)]),
                );
                ...
                let result = InferredTypeData::union_from_types(
                    self.db,
                    Vec::from([fulfillment_handler_value, rejection_handler_value]),
                );
                function(
                    Vec::from([fulfillment_handler_value, rejection_handler_value])
                        .into_boxed_slice(),
            ...
            PromiseInstanceMethod::Catch => {
                ...
                let result = InferredTypeData::union_from_types(
                    self.db,
                    Vec::from([receiver_value, rejection_handler_value]),
                );
                function(
                    Vec::from([rejection_handler_value]).into_boxed_slice(),
    ...
    /// Builds `Promise.resolve` with a result type derived from its argument.
    ///
    /// Other static Promise methods continue through the regular class-member
    /// lookup because their collection behavior requires separate modeling.
    fn promise_resolve_type(&self, target: InferredTypeData<'db>) -> InferredTypeData<'db> {
        ...
        let parameter = InferredFunctionParameter::Named(InferredNamedFunctionParameter {
