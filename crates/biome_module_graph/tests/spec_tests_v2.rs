...
fn test_infer_module_types_resolves_promise_member_chain() {
    ...
    for name in ["flattenedResolved", "flattenedAsync"] {
        let flattened = promise_value(name);
        assert!(
            contains_inferred_number(&db, flattened),
            "{name} must contain a number, got {}",
            format_inferred_type(&db, flattened)
        );
    }
    ...
    let awaited_then = inferred_binding_ty_by_name(&db, index_module, &inferred, "awaitedThen")
        .expect("awaited then value must be inferred");
    assert!(contains_inferred_number(
        &db,
        inferred.resolve_type(&db, awaited_then)
    ));
    for name in ["awaitedResolved", "awaitedAsync"] {
        let awaited = inferred_binding_ty_by_name(&db, index_module, &inferred, name)
            .expect("awaited flattened value must be inferred");
        assert!(contains_inferred_number(
            &db,
            inferred.resolve_type(&db, awaited)
        ));
    }
