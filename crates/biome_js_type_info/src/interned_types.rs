...
pub trait TypeDb: biome_db::Db {
    /// Returns the display name of a local type, if the module recorded one.
...
pub enum TypeData<'db> {
    ...
    /// A type stored in one module's inferred type table.
    ...
    /// The explicit TypeScript `any` keyword.
    AnyKeyword,
    NeverKeyword,
    ObjectKeyword,
    ThisKeyword,
...
impl<'db> TypeData<'db> {
    ...
    /// use biome_rowan::Text;
    ///
    /// # let db: &dyn TypeDb = todo!("provide a Salsa database");
...
/// Identifies one entry in one module's inferred type table.
#[salsa::interned]
#[derive(Debug)]
pub struct LocalTypeHandle<'db> {
    /// The module that owns the type table.
    pub module: ModuleKey,
    /// The entry's position in that module's type table.
...
mod tests {
    ...
    #[test]
    fn internal_type_descriptions_do_not_expose_salsa_data() {
        let db = TestDb::default();
        let generic = TypeData::Generic(InternedGenericTypeParameter::new(
            &db,
            None,
        ...
        assert_eq!(
            crate::InferredType::new(&db, generic).plus_operand_description(),
            "unknown"
        );
    }
